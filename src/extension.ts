...
export function activate(context: vscode.ExtensionContext) {
    ...
    let runDisposable = vscode.commands.registerCommand('ahmed-x86-asm.run', async () => {
        ...
        if (platform === 'linux') {
            ...
            if (linkerMethod === 'ld') {
                // أوامر الويندوز المقسمة (ld) مع إضافة cmd /c و --enable-stdcall-fixup
                switch (selectedIndex) {
                    case 1: commands = [`C:\\msys64\\mingw64\\bin\\uasm.exe -q -coff -I"${irvinePath}" -Fo"${baseName}.obj" "${fileName}"`, `cmd /c "C:\\msys64\\mingw32\\bin\\ld.exe \\"${baseName}.obj\\" \\"${path.join(irvinePath, 'Irvine32.lib')}\\" -o \\"${baseName}.exe\\" -lkernel32 -luser32 --subsystem console --enable-stdcall-fixup -L C:\\msys64\\mingw32\\lib"`, `.\\${baseName}.exe`]; break;
                    case 2: commands = [`C:\\msys64\\mingw64\\bin\\nasm.exe -f win32 "${fileName}" -o "${baseName}.obj"`, `cmd /c "C:\\msys64\\mingw32\\bin\\ld.exe \\"${baseName}.obj\\" -o \\"${baseName}.exe\\" -lkernel32 -luser32 --enable-stdcall-fixup -L C:\\msys64\\mingw32\\lib"`, `.\\${baseName}.exe`]; break;
                    case 3: commands = [`C:\\msys64\\mingw64\\bin\\nasm.exe -f win64 "${fileName}" -o "${baseName}.obj"`, `cmd /c "C:\\msys64\\mingw64\\bin\\ld.exe \\"${baseName}.obj\\" -o \\"${baseName}.exe\\" -lkernel32 -luser32 -L C:\\msys64\\mingw64\\lib"`, `.\\${baseName}.exe`]; break;
                    case 4: commands = [`C:\\msys64\\mingw64\\bin\\uasm.exe -q -coff -I"${irvinePath}" -Fo"${baseName}.obj" "${fileName}"`, `cmd /c "C:\\msys64\\mingw32\\bin\\ld.exe \\"${baseName}.obj\\" \\"${path.join(irvinePath, 'Irvine32.lib')}\\" -o \\"${baseName}.exe\\" -lkernel32 -luser32 -e _main --subsystem console --enable-stdcall-fixup -L C:\\msys64\\mingw32\\lib"`, `.\\${baseName}.exe`]; break;
                    case 5: commands = [`C:\\msys64\\mingw64\\bin\\nasm.exe -f win32 "${fileName}" -o "${baseName}.obj"`, `cmd /c "C:\\msys64\\mingw32\\bin\\ld.exe \\"${baseName}.obj\\" -o \\"${baseName}.exe\\" -lkernel32 -luser32 -e _main --enable-stdcall-fixup -L C:\\msys64\\mingw32\\lib"`, `.\\${baseName}.exe`]; break;
                    case 6: commands = [`C:\\msys64\\mingw64\\bin\\nasm.exe -f win64 "${fileName}" -o "${baseName}.obj"`, `cmd /c "C:\\msys64\\mingw64\\bin\\ld.exe \\"${baseName}.obj\\" -o \\"${baseName}.exe\\" -lkernel32 -luser32 -e main -L C:\\msys64\\mingw64\\lib"`, `.\\${baseName}.exe`]; break;
                }
            } else {
